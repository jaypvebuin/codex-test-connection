resource "aws_security_group" "sg" {
  ...
  name        = var.Environment == "beta" ? "${lower(var.vendor)}-${lower(var.project_name)}-${lower(var.identifier)}-sg-beta" : "${lower(var.vendor)}-${lower(var.project_name)}-${lower(var.identifier)}-sg"
