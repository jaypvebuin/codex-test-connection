resource "aws_lambda_permission" "image_lambda_permission" {
  # count         = var.required_external_invoke_permission == true ? 1 : 0
  count         = var.required_lambda_function && var.required_external_invoke_permission ? 1 : 0
  action        = var.action_on_lambda
  function_name = aws_lambda_function.image_lambda_function[0].function_name
