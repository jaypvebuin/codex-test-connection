...
locals {
  pdf_codebuild_config = [
    # {
    #   identifier                  = "pdf-owasp"
    ...
    #       security_group_ids = [module.pdf-codebuild_sg["pdf-owasp"].sg_id]
    #     }
    #   ]
    ...
    #     {
    #       name  = "project_name",
    #       value = "vb-jugaad-pdf-service",
    #       type  = "PLAINTEXT"
    #     }
    #   ]
    # },
    {
      identifier                  = "pdf-sonarqube"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.pdf-codebuild_sg["pdf-sonarqube"].sg_id],null)
        }
      ]
    ...
    },
    {
      ...
      codebuild_role_policy_arn = ["${module.iam_policy["pdf-codebuild"].policy_arn}"]

      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = [module.pdf-codebuild_sg["pdf-app"].sg_id]
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "container_name",
          value = "vb-jugaad-pdf-task-definition-${var.environment}-container",
          type  = "PLAINTEXT"
        },
        {
          name  = "ecr_repo",
          value = "vb-jugaad-pdf-service-repo-${var.environment}",
          type  = "PLAINTEXT"
        },
        {
          name  = "SERVICE_NAME",
  ...
  pdf_codebuild_configs_map = { for idx, config in local.pdf_codebuild_config : config.identifier => config }
}

# codebuild module

module "pdf-codebuild" {
  for_each = var.microservice_resources[var.environment] > 0 ? {
    for identifier, config in local.pdf_codebuild_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "pdf-sonarqube" || identifier == "pdf-app")) ||
      (var.environment != "dev" && identifier == "pdf-app")
    )
  } : {}
  ...
  depends_on = [module.pdf-codebuild_sg]
...
locals {
      ...
      Statement =[
       {
        ...
        Action    = "s3:*",
        Resource = [
          "arn:aws:s3:::vb-jugaad-pdf-service-pipeline-artifact-bucket-${lower(var.environment)}/*",
          "arn:aws:s3:::vb-jugaad-pdf-service-pipeline-artifact-bucket-${lower(var.environment)}"
        ],
        Condition = {
          Bool = {
    ...
    stages = [
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              FullRepositoryId = "vebuin/jugaad-form-export"
      ...
      var.environment == "dev" ? {
        ...
        actions = [
          ...
          #   configuration = {
          #     ProjectName = module.pdf-codebuild["pdf-owasp"].codebuild_name
          #   }
          # },
          {
            ...
            configuration = {
              ProjectName = module.pdf-codebuild["pdf-sonarqube"].codebuild_name
            }
          }
        ]
      } : null,
        ...
        actions = [
          {
            ...
            configuration = {
              ProjectName = module.pdf-codebuild["pdf-app"].codebuild_name
            }
          }
        ]
      },
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              ServiceName = "vb-jugaad-pdf-service-${lower(var.environment)}"
  ...
  pdf_pipeline_configs_map = { for idx, config in local.pdf_pipeline_configs : config.identifier => config }
}
...
module "pdf-pipeline" {
  ...
  for_each = var.microservice_resources[var.environment] > 0 ? local.pdf_pipeline_configs_map : {}
...
locals {
  ...
  pdf_sg_configs_map = { for idx, config in local.pdf_sg_configs : config.identifier => config }
}

module "pdf-codebuild_sg" {
  source   = "../modules/CodePipeline/codebuild_sg"
  # for_each = var.microservice_resources[var.environment] > 0 ? local.pdf_sg_configs_map : {}
  for_each = var.microservice_resources[var.environment] > 0 ? {
    for identifier, config in local.pdf_sg_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "pdf-sonarqube" || identifier == "pdf-app")) ||
      (var.environment != "dev" && identifier == "pdf-app")
    )
