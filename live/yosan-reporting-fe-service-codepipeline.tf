locals {
  yosan_reporting_fe_codebuild_config = [
    # {
    ...
    # },
    {
      identifier                  = "yosan-reporting-fe-sonarqube"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.yosan-reporting-fe-codebuild_sg["yosan-reporting-fe-sonarqube"].sg_id],[])
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "SERVICE_NAME",
          value = "yosan-reporting-fe-service",
          type  = "PLAINTEXT"
        },
        {
          name  = "sonar_project_name",
          value = "vb-jugaad-yosan-reporting-fe-service",
          type  = "PLAINTEXT"
        }
      ]
    },
    {
      identifier                  = "yosan-reporting-fe-app"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.yosan-reporting-fe-codebuild_sg["yosan-reporting-fe-app"].sg_id],[])
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "container_name",
          value = "vb-jugaad-yosan-reporting-fe-task-definition-${var.environment}-container",
          type  = "PLAINTEXT"
        },
        {
          name  = "ecr_repo",
          value = "vb-jugaad-yosan-reporting-fe-service-repo-${var.environment}",
          type  = "PLAINTEXT"
        },
        {
          name  = "SERVICE_NAME",
        ...
        },
        {
          ...
          value = "REPORTING_FE_VITE_REACT_APP_LOGIN_URL",
          type  = "PARAMETER_STORE"
        },
        {
          name  = "VITE_KEYCLOAK_URL",
          value = "REPORTING_FE_VITE_KEYCLOAK_URL",
          type  = "PARAMETER_STORE"
        },
        {
          name  = "VITE_KEYCLOAK_REALM",
          value = "REPORTING_FE_VITE_KEYCLOAK_REALM",
          type  = "PARAMETER_STORE"
        },
        {
          name  = "VITE_KEYCLOAK_CLIENT_ID",
          value = "REPORTING_FE_VITE_KEYCLOAK_CLIENT_ID",
          type  = "PARAMETER_STORE"
        },
        {
          name  = "VITE_REPORT_API_BASE_URL",
          value = "REPORTING_FE_VITE_REPORT_API_BASE_URL",
          type  = "PARAMETER_STORE"
        },
        {
          name  = "VITE_COMMON_API_URL",
          value = "REPORTING_FE_VITE_COMMON_API_URL",
          type  = "PARAMETER_STORE"
        }
      ]
    }
  ]
  yosan_reporting_fe_codebuild_configs_map = { for idx, config in local.yosan_reporting_fe_codebuild_config : config.identifier => config }
}

# codebuild module

module "yosan-reporting-fe-codebuild" {
  for_each = var.yosan_resources[var.environment] > 0 ? {
    for identifier, config in local.yosan_reporting_fe_codebuild_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "yosan-reporting-fe-sonarqube" || identifier == "yosan-reporting-fe-app")) ||
      (var.environment != "dev" && identifier == "yosan-reporting-fe-app")
    )
  } : {}
  ...
  depends_on = [module.yosan-reporting-fe-codebuild_sg]
...
locals {
  yosan_reporting_fe_pipeline_configs = [{
    identifier = "yosan-report-fe-service"
    #------------ artifact_bucket.tf ---------------
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              FullRepositoryId = "vebuin/jugaad-pivot-report-fe"
        ...
        var.environment == "dev" ? {
          ...
          actions = [
            ...
            # },
            {
              ...
              configuration = {
                ProjectName = module.yosan-reporting-fe-codebuild["yosan-reporting-fe-sonarqube"].codebuild_name
              }
            }
          ]
        } : null,
        ...
        actions = [
          {
            ...
            configuration = {
              ProjectName = try(module.yosan-reporting-fe-codebuild["yosan-reporting-fe-app"].codebuild_name,null)
            }
          }
        ]
      }
    ...
    #           ServiceName = "vb-jugaad-yosan-reporting-fe-service-${lower(var.environment)}"
  ...
  yosan_reporting_fe_pipeline_configs_map = { for idx, config in local.yosan_reporting_fe_pipeline_configs : config.identifier => config }
}

# codepipeline module

module "yosan-reporting-fe-pipeline" {
  source   = "../modules/CodePipeline/pipeline"
  for_each = var.yosan_resources[var.environment] > 0 ? local.yosan_reporting_fe_pipeline_configs_map : {}
...
locals {
  yosan_reporting_fe_sg_configs = [
    {
      identifier              = "yosan-reporting-fe-owasp"
  ...
  yosan_reporting_fe_sg_configs_map = { for idx, config in local.yosan_reporting_fe_sg_configs : config.identifier => config }
}

module "yosan-reporting-fe-codebuild_sg" {
  source   = "../modules/CodePipeline/codebuild_sg"
  # for_each = var.yosan_resources[var.environment] > 0 ? local.yosan_reporting_fe_sg_configs_map : {}
  for_each = var.yosan_resources[var.environment] > 0 ? {
    for identifier, config in local.yosan_reporting_fe_sg_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "yosan-reporting-fe-sonarqube" || identifier == "yosan-reporting-fe-owasp" || identifier == "yosan-reporting-fe-app")) ||
      (var.environment != "dev" && identifier == "yosan-reporting-fe-app")
    )
  } : {}
