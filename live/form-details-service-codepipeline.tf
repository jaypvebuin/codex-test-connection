...
locals {
  form_details_codebuild_config = [
    ...
    # },
    {
      identifier                  = "form-details-sonarqube"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.form-details-codebuild_sg["form-details-sonarqube"].sg_id],null)
        }
      ]
    ...
    },
    {
      ...
      codebuild_role_policy_arn = ["${module.iam_policy["form-details-codebuild"].policy_arn}"]

      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = [module.form-details-codebuild_sg["form-details-app"].sg_id]
        }
      ]
  ...
  form_details_codebuild_configs_map = { for idx, config in local.form_details_codebuild_config : config.identifier => config }
}

# codebuild module

module "form-details-codebuild" {
  for_each = var.microservice_resources[var.environment] > 0 ? {
    for identifier, config in local.form_details_codebuild_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "form-details-sonarqube" || identifier == "form-details-app")) ||
      (var.environment != "dev" && identifier == "form-details-app")
    )
  } : {}
  ...
  depends_on = [module.form-details-codebuild_sg]
...
locals {
      ...
      Statement =[
       {
        ...
        Action    = "s3:*",
        Resource = [
          "arn:aws:s3:::vb-jugaad-form-details-service-pipeline-artifact-bucket-${lower(var.environment)}/*",
          "arn:aws:s3:::vb-jugaad-form-details-service-pipeline-artifact-bucket-${lower(var.environment)}"
        ],
        Condition = {
          Bool = {
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              FullRepositoryId = "vebuin/jugaad-form-details"
      ...
      var.environment == "dev" ? {
        ...
        actions = [
          ...
          # },
          {
            ...
            configuration = {
              ProjectName = module.form-details-codebuild["form-details-sonarqube"].codebuild_name
            }
          }
        ]
      } : null,
        ...
        actions = [
          {
            ...
            configuration = {
              ProjectName = module.form-details-codebuild["form-details-app"].codebuild_name
            }
          }
        ]
      },
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              ServiceName = "vb-jugaad-form-details-service-${lower(var.environment)}"
  ...
  form_details_pipeline_configs_map = { for idx, config in local.form_details_pipeline_configs : config.identifier => config }
}

# codepipeline module

module "form-details-pipeline" {
  source   = "../modules/CodePipeline/pipeline"
  for_each = var.microservice_resources[var.environment] > 0 ? local.form_details_pipeline_configs_map : {}
...
locals {
  ...
  form_details_sg_configs_map = { for idx, config in local.form_details_sg_configs : config.identifier => config }
}

module "form-details-codebuild_sg" {
  source   = "../modules/CodePipeline/codebuild_sg"
  # for_each = var.microservice_resources[var.environment] > 0 ? local.form_details_sg_configs_map : {}
  for_each = var.microservice_resources[var.environment] > 0 ? {
    for identifier, config in local.form_details_sg_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "form-details-sonarqube" || identifier == "form-details-app")) ||
      (var.environment != "dev" && identifier == "form-details-app")
    )
