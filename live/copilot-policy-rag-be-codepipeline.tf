...
locals {
  cp_policy_rag_codebuild_config = [
    # {
    #   identifier                  = "cp-policy-rag-owasp"
    ...
    #       security_group_ids = [module.cp-policy-rag-codebuild_sg["cp-policy-rag-owasp"].sg_id]
    #     }
    #   ]
    ...
    #     {
    #       name  = "SERVICE_NAME",
    #       value = "cp-policy-rag-service",
    #       type  = "PLAINTEXT"
    #     },
    #     {
    #       name  = "project_name",
    #       value = "vb-jugaad-cp-policy-rag-service",
    #       type  = "PLAINTEXT"
    #     }
    #   ]
    # },
    {
      identifier                  = "cp-policy-rag-sonarqube"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.cp-policy-rag-codebuild_sg["cp-policy-rag-sonarqube"].sg_id],[])
        }
      ]
      ...
      environment_variable = [
        ...
        },
        {
          ...
          value = "vb-jugaad-copilot-policy-rag-service-be",
          type  = "PLAINTEXT"
        }
      ]
    },
    {
      identifier                  = "cp-policy-rag-app"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.cp-policy-rag-codebuild_sg["cp-policy-rag-app"].sg_id],[])
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "container_name",
          value = "vb-jugaad-cp-policy-rag-task-definition-${var.environment}-container",
          type  = "PLAINTEXT"
        },
        {
          name  = "ecr_repo",
          value = "vb-jugaad-cp-policy-rag-service-repo-${var.environment}",
          type  = "PLAINTEXT"
        },
        {
          name  = "SERVICE_NAME",
  ...
  cp_policy_rag_codebuild_configs_map = { for idx, config in local.cp_policy_rag_codebuild_config : config.identifier => config }
}

# codebuild module

module "cp-policy-rag-codebuild" {
  for_each = var.copilot_resources[var.environment] > 0 ? {
    for identifier, config in local.cp_policy_rag_codebuild_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "cp-policy-rag-sonarqube" || identifier == "cp-policy-rag-app")) ||
      (var.environment != "dev" && identifier == "cp-policy-rag-app")
    )
  } : {}
  ...
  depends_on = [module.cp-policy-rag-codebuild_sg]
...
locals {
  cp_policy_rag_pipeline_configs = [{
    identifier = "cp-policy-rag-service"
    #------------ artifact_bucket.tf ---------------
    ...
    stages = [
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              FullRepositoryId = "vebuin/jugaad-copilot"
              #   BranchName       = var.branch_name[var.environment]
              BranchName = var.copilot_rag_branch_name
            }
          }
        ]
      },
      var.environment == "dev" ? {
        ...
        actions = [
          ...
          #   configuration = {
          #     ProjectName = module.cp-policy-rag-codebuild["cp-policy-rag-owasp"].codebuild_name
          #   }
          # },
          {
            ...
            configuration = {
              ProjectName = module.cp-policy-rag-codebuild["cp-policy-rag-sonarqube"].codebuild_name
            }
          }
        ]
      } : null,
        ...
        actions = [
          {
            ...
            configuration = {
              ProjectName = try(module.cp-policy-rag-codebuild["cp-policy-rag-app"].codebuild_name,null)
            }
          }
        ]
      },
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              ServiceName = "vb-jugaad-copilot-policy-rag-service-${lower(var.environment)}"
  ...
  cp_policy_rag_pipeline_configs_map = { for idx, config in local.cp_policy_rag_pipeline_configs : config.identifier => config }
}

# codepipeline module

module "cp-policy-rag-pipeline" {
  source   = "../modules/CodePipeline/pipeline"
  for_each = var.copilot_resources[var.environment] > 0 ? local.cp_policy_rag_pipeline_configs_map : {}
...
locals {
  ...
  cp_policy_rag_sg_configs_map = { for idx, config in local.cp_policy_rag_sg_configs : config.identifier => config }
}

module "cp-policy-rag-codebuild_sg" {
  source   = "../modules/CodePipeline/codebuild_sg"
  # for_each = var.copilot_resources[var.environment] > 0 ? local.cp_policy_rag_sg_configs_map : {}
  for_each = var.copilot_resources[var.environment] > 0 ? {
    for identifier, config in local.cp_policy_rag_sg_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "cp-policy-rag-sonarqube" || identifier == "cp-policy-rag-app")) ||
      (var.environment != "dev" && identifier == "cp-policy-rag-app")
    )
