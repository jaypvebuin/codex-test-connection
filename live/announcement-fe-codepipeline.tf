...
locals {
  announcement_fe_codebuild_config = [
    {
      identifier                  = "announcement-fe-sonarqube"
      ...
      vpc_config = [
        {
          ...
          security_group_ids = try([module.announcement_fe_codebuild_sg["announcement-fe-sonarqube"].sg_id],[])
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "SERVICE_NAME",
          value = "announcement-fe",
          type  = "PLAINTEXT"
        },
        {
          name  = "project_name",
          value = "vb-jugaad-announcement-service-fe",
          type  = "PLAINTEXT"
        }
      ]
    },
    {
      identifier                  = "announcement-fe-app"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:7.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.announcement_fe_codebuild_sg["announcement-fe-app"].sg_id],[])
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "project_name",
          value = "vb-jugaad-announcement-fe",
          type  = "PLAINTEXT"
        },
        {
          name  = "account_id",
  ...
  announcement_fe_codebuild_configs_map = { for idx, config in local.announcement_fe_codebuild_config : config.identifier => config }
}

# codebuild module

module "announcement_fe_codebuild" {
#   count = var.announcement_resources[var.environment]
  for_each = var.announcement_resources[var.environment] > 0 ? {
    for identifier, config in local.announcement_fe_codebuild_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "announcement-fe-sonarqube" || identifier == "announcement-fe-app")) ||
      (var.environment != "dev" && identifier == "announcement-fe-app")
    )
  } : {}
  ...
  depends_on = [module.announcement_fe_codebuild_sg]
...
locals {
  announcement_fe_pipeline_configs = [{
    identifier = "announcement-fe"
    #------------ artifact_bucket.tf ---------------
    ...
    stages = [
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              FullRepositoryId = "vebuin/jugaad-announcementservice-fe"
      ...
      var.environment == "dev" ? {
        ...
        actions = [
          ...
          #   configuration = {
          #     ProjectName = module.codebuild["announcement-owasp"].codebuild_name
          #   }
          # },
          {
            ...
            configuration = {
              ProjectName = try(module.announcement_fe_codebuild["announcement-fe-sonarqube"].codebuild_name,null)
            }
          }
        ]
      } : null,
        ...
        actions = [
          {
            ...
            configuration = {
              ProjectName = try(module.announcement_fe_codebuild["announcement-fe-app"].codebuild_name,null)
            }
          }
        ]
      }
      ...
      #           ServiceName = "vb-jugaad-announcement-service-dev"
  ...
  announcement_fe_pipeline_configs_map = { for idx, config in local.announcement_fe_pipeline_configs : config.identifier => config }
}

# codepipeline module

module "announcement_fe_pipeline" {
#   count = var.announcement_resources[var.environment]
  source   = "../modules/CodePipeline/pipeline"
  for_each = var.announcement_resources[var.environment] > 0 ? local.announcement_fe_pipeline_configs_map : {}
...
locals {
  ...
  announcement_fe_sg_configs_map = { for idx, config in local.announcement_fe_sg_configs : config.identifier => config }
}

module "announcement_fe_codebuild_sg" {
#   count = var.announcement_resources[var.environment]
  source   = "../modules/CodePipeline/codebuild_sg"
  for_each = var.announcement_resources[var.environment] > 0 ? local.announcement_fe_sg_configs_map : {}
