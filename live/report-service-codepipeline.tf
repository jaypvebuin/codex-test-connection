...
locals {
  report_codebuild_config = [
    # {
    #   identifier                  = "report-owasp"
    ...
    #   codebuild_role_policy_arn = ["arn:aws:iam::aws:policy/AdministratorAccess","${module.iam_policy["sonarqube-codebuild"].policy_arn}"]

    #   vpc_config = [
    #     {
    #       vpc_id             = var.vpc_id[var.environment],
    #       subnets            = var.subnet_ids[var.environment],
    #       security_group_ids = [module.report_codebuild_sg["report-owasp"].sg_id]
    #     }
    #   ]
    ...
    #     {
    #       name  = "project_name",
    #       value = "vb-jugaad-report-service",
    #       type  = "PLAINTEXT"
    #     }
    #   ]
    # },
    {
      identifier                  = "report-sonarqube"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.report_codebuild_sg["report-sonarqube"].sg_id],null)
        }
      ]
    ...
    },
    {
      ...
      codebuild_role_policy_arn = ["${module.iam_policy["report-codebuild"].policy_arn}"]

      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = [module.report_codebuild_sg["report-app"].sg_id]
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "ecr_repo",
          value = "vb-jugaad-report-service-repo-${var.environment}",
          type  = "PLAINTEXT"
        },
        {
          name  = "account_id",
        ...
        {
          name  = "container_name",
          value = "vb-jugaad-report-task-definition-${var.environment}-container",
          type  = "PLAINTEXT"
        }
      ]
    }
  ]
  report_codebuild_configs_map = { for idx, config in local.report_codebuild_config : config.identifier => config }
}

# codebuild module

module "report_codebuild" {
  for_each = var.microservice_resources[var.environment] > 0 ? {
    for identifier, config in local.report_codebuild_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "report-sonarqube" || identifier == "report-app")) ||
      (var.environment != "dev" && identifier == "report-app")
    )
  } : {}
  ...
  depends_on = [module.report_codebuild_sg]
...
locals {
      ...
      Statement =[
       {
        ...
        Action    = "s3:*",
        Resource = [
          "arn:aws:s3:::vb-jugaad-report-service-pipeline-artifact-bucket-${lower(var.environment)}/*",
          "arn:aws:s3:::vb-jugaad-report-service-pipeline-artifact-bucket-${lower(var.environment)}"
        ],
        Condition = {
          Bool = {
    ...
    stages = [
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              FullRepositoryId = "vebuin/jugaad-report"
      ...
      var.environment == "dev" ? {
        ...
        actions = [
          ...
          #   configuration = {
          #     ProjectName = module.report_codebuild["report-owasp"].codebuild_name
          #   }
          # },
          {
            ...
            configuration = {
              ProjectName = module.report_codebuild["report-sonarqube"].codebuild_name
            }
          }
        ]
      } : null,
        ...
        actions = [
          {
            ...
            configuration = {
              ProjectName = module.report_codebuild["report-app"].codebuild_name
            }
          }
        ]
      },
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              ServiceName = "vb-jugaad-report-service-${lower(var.environment)}"
  ...
  report_pipeline_configs_map = { for idx, config in local.report_pipeline_configs : config.identifier => config }
}

# codepipeline module

module "report_pipeline" {
  source   = "../modules/CodePipeline/pipeline"
  for_each = var.microservice_resources[var.environment] > 0 ? local.report_pipeline_configs_map : {}
...
locals {
  ...
  report_sg_configs_map = { for idx, config in local.report_sg_configs : config.identifier => config }
}

module "report_codebuild_sg" {
  source   = "../modules/CodePipeline/codebuild_sg"
  # for_each = var.microservice_resources[var.environment] > 0 ? local.report_sg_configs_map : {}
  for_each = var.microservice_resources[var.environment] > 0 ? {
    for identifier, config in local.report_sg_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "report-sonarqube" || identifier == "report-app")) ||
      (var.environment != "dev" && identifier == "report-app")
    )
