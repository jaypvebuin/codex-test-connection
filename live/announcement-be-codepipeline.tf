...
locals {
  announcement_codebuild_config = [
    # {
    #   identifier                  = "announcement-owasp"
    ...
    #       security_group_ids = [module.announcement-codebuild_sg["announcement-owasp"].sg_id]
    #     }
    #   ]
    ...
    #     {
    #       name  = "project_name",
    #       value = "vb-jugaad-announcement-service",
    #       type  = "PLAINTEXT"
    #     }
    #   ]
    # },
    {
      identifier                  = "announcement-sonarqube"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.announcement-codebuild_sg["announcement-sonarqube"].sg_id],[])
        }
      ]
    ...
    },
    {
      ...
      codebuild_role_policy_arn = ["${module.iam_policy["announcement-be-codebuild"].policy_arn}"]

      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.announcement-codebuild_sg["announcement-app"].sg_id],[])
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "container_name",
          value = "vb-jugaad-announcement-task-definition-${var.environment}-container",
          type  = "PLAINTEXT"
        },
        {
          name  = "ecr_repo",
          value = "vb-jugaad-announcement-service-repo-${var.environment}",
          type  = "PLAINTEXT"
        },
        {
          name  = "SERVICE_NAME",
        ...
        {
          name  = "project_name",
          value = "vb-jugaad-announcement-service-be",
          type  = "PLAINTEXT"
        },
        {
          name  = "account_id",
  ...
  announcement_codebuild_configs_map = { for idx, config in local.announcement_codebuild_config : config.identifier => config }
}

# codebuild module

module "announcement-codebuild" {
#   count = var.announcement_resources[var.environment]
  for_each = var.announcement_resources[var.environment] > 0 ? {
    for identifier, config in local.announcement_codebuild_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "announcement-sonarqube" || identifier == "announcement-app")) ||
      (var.environment != "dev" && identifier == "announcement-app")
    )
  } : {}
  ...
  depends_on = [module.announcement-codebuild_sg]
...
locals {
    ...
    stages = [
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              FullRepositoryId = "vebuin/Jugaad-AnnouncementService-BE"
              BranchName       = "main"
      ...
      var.environment == "dev" ? {
        ...
        actions = [
          ...
          #   configuration = {
          #     ProjectName = module.announcement-codebuild["announcement-owasp"].codebuild_name
          #   }
          # },
          {
            ...
            configuration = {
              ProjectName = try(module.announcement-codebuild["announcement-sonarqube"].codebuild_name,null)
            }
          }
        ]
      } : null,
        ...
        actions = [
          {
            ...
            configuration = {
              ProjectName = try(module.announcement-codebuild["announcement-app"].codebuild_name,null)
            }
          }
        ]
      },
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              ServiceName = "vb-jugaad-announcement-service-${lower(var.environment)}"
  ...
  announcement_pipeline_configs_map = { for idx, config in local.announcement_pipeline_configs : config.identifier => config }
}

# codepipeline module

module "announcement-pipeline" {
#   count = var.announcement_resources[var.environment]
  source   = "../modules/CodePipeline/pipeline"
  for_each = var.announcement_resources[var.environment] > 0 ? local.announcement_pipeline_configs_map : {}
...
locals {
  ...
  announcement_sg_configs_map = { for idx, config in local.announcement_sg_configs : config.identifier => config }
}

module "announcement-codebuild_sg" {
#   count = var.announcement_resources[var.environment]
  source   = "../modules/CodePipeline/codebuild_sg"
  for_each = var.announcement_resources[var.environment] > 0 ? local.announcement_sg_configs_map : {}
