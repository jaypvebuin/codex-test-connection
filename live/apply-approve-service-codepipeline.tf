...
locals {
  codebuild_config = [
    {
      identifier                  = "apply-approve-sonarqube"
      ...
      vpc_config = [
        {
          ...
          security_group_ids = try([module.codebuild_sg["apply-approve-sonarqube"].sg_id], null)
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "SERVICE_NAME",
          value = "apply-approve-service",
          type  = "PLAINTEXT"
        },
        {
          name  = "sonar_project_name",
          value = "vb-jugaad-apply-approve-service",
          type  = "PLAINTEXT"
        }
      ]
    },
    {
      identifier                  = "apply-approve-app"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      codebuild_role_policy_arn = ["${module.iam_policy["apply-approve-codebuild"].policy_arn}"]

      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = [module.codebuild_sg["apply-approve-app"].sg_id]
        }
      ]
  ...
  codebuild_configs_map = { for idx, config in local.codebuild_config : config.identifier => config }
}
...
module "codebuild" {
  ...
  # for_each = {
  #   for identifier, config in local.codebuild_configs_map :
  #   identifier => config if(
  #     (var.environment == "dev" && (identifier == "apply-approve-sonarqube" || identifier == "apply-approve-app")) ||
  #     (var.environment != "dev" && identifier == "apply-approve-app")
  #   )
  # }
  ...
  depends_on = [module.codebuild_sg, module.iam_policy]
...
locals {
      ...
      Statement =[
       {
        ...
        Action    = "s3:*",
        Resource = [
          "arn:aws:s3:::vb-jugaad-apply-approve-service-pipeline-artifact-bucket-${lower(var.environment)}/*",
          "arn:aws:s3:::vb-jugaad-apply-approve-service-pipeline-artifact-bucket-${lower(var.environment)}"
        ],
        Condition = {
          Bool = {
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              FullRepositoryId = "vebuin/jugaad-apply-approve"
      ...
      var.environment == "dev" ? {
        ...
        actions = [
          ...
          # },
          {
            ...
            configuration = {
              ProjectName = module.codebuild["apply-approve-sonarqube"].codebuild_name
            }
          }
        ]
      } : null,
        ...
        actions = [
          {
            ...
            configuration = {
              ProjectName = module.codebuild["apply-approve-app"].codebuild_name
            }
          }
        ]
      },
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              ServiceName = "vb-jugaad-apply-approve-service-${lower(var.environment)}"
  ...
  pipeline_configs_map = { for idx, config in local.pipeline_configs : config.identifier => config }
}
...
module "pipeline" {
  ...
  # for_each = local.pipeline_configs_map
  for_each = var.microservice_resources[var.environment] > 0 ? local.pipeline_configs_map : {}
...
locals {
  ...
  sg_configs_map = { for idx, config in local.sg_configs : config.identifier => config }
}

module "codebuild_sg" {
  # count = var.microservice_resources[var.environment]
  source   = "../modules/CodePipeline/codebuild_sg"
  # for_each = local.sg_configs_map
  # for_each = var.microservice_resources[var.environment] > 0 ? local.sg_configs_map : {}
  for_each = var.microservice_resources[var.environment] > 0 ? {
    for identifier, config in local.sg_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "apply-approve-sonarqube" || identifier == "apply-approve-app")) ||
      (var.environment != "dev" && identifier == "apply-approve-app")
    )
