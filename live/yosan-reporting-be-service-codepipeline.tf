locals {
  yosan_reporting_be_codebuild_config = [
    # {
    ...
    # },
    {
      identifier                  = "yosan-reporting-be-sonarqube"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.yosan-reporting-be-codebuild_sg["yosan-reporting-be-sonarqube"].sg_id],[])
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "SERVICE_NAME",
          value = "yosan-reporting-be-service",
          type  = "PLAINTEXT"
        },
        {
          name  = "sonar_project_name",
          value = "vb-jugaad-yosan-reporting-be-service",
          type  = "PLAINTEXT"
        }
      ]
    },
    {
      identifier                  = "yosan-reporting-be-app"
      build_compute_type          = "BUILD_GENERAL1_SMALL"
      build_environment_image     = "aws/codebuild/standard:6.0"
      build_environment_type      = "LINUX_CONTAINER"
      image_pull_credentials_type = "CODEBUILD"
      ...
      vpc_config = [
        {
          vpc_id             = var.vpc_id[var.environment],
          subnets            = var.subnet_ids[var.environment],
          security_group_ids = try([module.yosan-reporting-be-codebuild_sg["yosan-reporting-be-app"].sg_id],[])
        }
      ]
      ...
      environment_variable = [
        ...
        {
          name  = "container_name",
          value = "vb-jugaad-yosan-reporting-be-task-definition-${var.environment}-container",
          type  = "PLAINTEXT"
        },
        {
          name  = "ecr_repo",
          value = "vb-jugaad-yosan-reporting-be-service-repo-${var.environment}",
          type  = "PLAINTEXT"
        },
        {
          name  = "SERVICE_NAME",
  ...
  yosan_reporting_be_codebuild_configs_map = { for idx, config in local.yosan_reporting_be_codebuild_config : config.identifier => config }
}

# codebuild module

module "yosan-reporting-be-codebuild" {
  for_each = var.yosan_resources[var.environment] > 0 ? {
    for identifier, config in local.yosan_reporting_be_codebuild_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "yosan-reporting-be-sonarqube" || identifier == "yosan-reporting-be-app")) ||
      (var.environment != "dev" && identifier == "yosan-reporting-be-app")
    )
  } : {}
  ...
  depends_on = [module.yosan-reporting-be-codebuild_sg]
...
locals {
  yosan_reporting_be_pipeline_configs = [{
    identifier = "yosan-report-be-service"
    #------------ artifact_bucket.tf ---------------
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              FullRepositoryId = "vebuin/jugaad-pivot-report-be"
        ...
        var.environment == "dev" ? {
          ...
          actions = [
            ...
            # },
            {
              ...
              configuration = {
                ProjectName = module.yosan-reporting-be-codebuild["yosan-reporting-be-sonarqube"].codebuild_name
              }
            }
          ]
        } : null,
        ...
        actions = [
          {
            ...
            configuration = {
              ProjectName = try(module.yosan-reporting-be-codebuild["yosan-reporting-be-app"].codebuild_name, null)
            }
          }
        ]
      },
        ...
        actions = [
          {
            ...
            configuration = {
              ...
              ServiceName = "vb-jugaad-yosan-reporting-be-service-${lower(var.environment)}"
  ...
  yosan_reporting_be_pipeline_configs_map = { for idx, config in local.yosan_reporting_be_pipeline_configs : config.identifier => config }
}

# codepipeline module

module "yosan-reporting-be-pipeline" {
  source   = "../modules/CodePipeline/pipeline"
  for_each = var.yosan_resources[var.environment] > 0 ? local.yosan_reporting_be_pipeline_configs_map : {}
...
locals {
  yosan_reporting_be_sg_configs = [
    {
      identifier              = "yosan-reporting-be-owasp"
  ...
  yosan_reporting_be_sg_configs_map = { for idx, config in local.yosan_reporting_be_sg_configs : config.identifier => config }
}

module "yosan-reporting-be-codebuild_sg" {
  source   = "../modules/CodePipeline/codebuild_sg"
  # for_each = var.yosan_resources[var.environment] > 0 ? local.yosan_reporting_be_sg_configs_map : {}
  for_each = var.yosan_resources[var.environment] > 0 ? {
    for identifier, config in local.yosan_reporting_be_sg_configs_map :
    identifier => config if(
      (var.environment == "dev" && (identifier == "yosan-reporting-be-sonarqube" || identifier == "yosan-reporting-be-owasp" || identifier == "yosan-reporting-be-app")) ||
      (var.environment != "dev" && identifier == "yosan-reporting-be-app")
    )
  } : {}
